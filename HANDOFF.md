...
- **音声**: ElevenLabs **Kana**（ネイティブ日本語の若い女性声）を**事前生成**して同梱（ランタイムはキー不要・課金なし・オフライン）。音声URLは `?v=<声ID>` でキャッシュ無効化。
- **iOS対応済み**: 単一Audio使い回しで自動再生対策・「ホーム画面に追加」で全画面アプリ表示（manifest.webmanifest）。
...
（既定 voice=**Kana**（ネイティブ日本語女性）`dhGvgIx0X6G3xzSWqOye`・speed 0.92。日本語候補は ElevenLabs Voice Library language=ja。**声を変えたら必ず `public/audio/*.mp3` を全削除してから再生成**＝同名ファイルはスキップされるため）
...
## 10. コミット履歴（要点・新しい順）
```
fcf7cd6 docs: HANDOFF の声情報を Kana に更新
b9a5bf7 fix(audio): 音声URLに声IDのキャッシュバスター付与
a7b3c2d feat(voice): 音声をネイティブ日本語の女性声 Kana に変更
a0ba943 feat(ios): iOS Safari で確実に音が鳴る + ホーム画面追加対応
205dfc4 docs: HANDOFF.md 追加（引き継ぎ・作業再開ガイド）
